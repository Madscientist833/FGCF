...
class Part_Attention(nn.Module):
    def __init__(self, fusion_way='mul'):
    ...
    def forward(self, x):
        length = len(x)
        last_map = x[0]
        '''Several ways to combine attention are provided here'''
        ...
        '''Choose the largest K patches as output'''
        last_map = last_map[:,:,0,1:]
        _, max_inx = last_map.max(2)
        return _, max_inx

class Encoder(nn.Module):
    def __init__(self, config, fusion_way='mul'):
        super(Encoder, self).__init__()
        self.layer = nn.ModuleList()
        for _ in range(config.transformer["num_layers"] - 1):
            layer = Block(config)
            self.layer.append(copy.deepcopy(layer))
        self.part_select = Part_Attention(fusion_way)
    ...
    def forward(self, hidden_states):
        ...
        #all_hidden_state=[i for i in range(1,325)]
        for i in range(B):
            parts.append(hidden_states[i, part_inx[i,:]])#all_hidden_state
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, fusion_way='mul'):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size)
        self.encoder = Encoder(config, fusion_way)
        self.fusion_way = fusion_way
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, fusion_way='mul'):
        super(VisionTransformer, self).__init__()
        self.num_classes = num_classes
        self.smoothing_value = smoothing_value
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size ,fusion_way)
